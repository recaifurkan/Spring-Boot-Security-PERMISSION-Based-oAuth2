...
public class UserController {
	...
	@PutMapping(value = "/{id}")
	public UserApiResponse updateUser(@PathVariable long id, @RequestBody UserDto user) throws Exception {
		log.info(String.format("received request to update user %s",
				authenticationFacadeService.getAuthentication().getPrincipal()));
		return new UserApiResponse(HttpStatus.OK, Status.SUCCESS.name(), userService.updateUser(id, user));
	}
